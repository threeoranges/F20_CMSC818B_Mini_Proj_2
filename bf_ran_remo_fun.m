% brute-force random remo

function [N_bf_max_cover, bf_remain_ran_remo, ran_remo_rate_bf] = bf_ran_remo_fun(Nr, N_direction,...
    ...
    for i = 1:Nr
        ...
        if ismember(robot_inx, r_ran_remo) > 0
           s_tra_cover_ran_remain{i+1} = s_tra_cover_ran_remain{i}; 
        else
           s_tra_cover_ran_remain{i+1} = union(s_tra_cover_ran_remain{i}, tar_cover{robot_inx,tra_inx});
        end
    end
    N_bf_maxmin_cover = length(s_tra_cover_bf{Nr+1}); %max min cover 
    bf_remain_ran_remo = length(s_tra_cover_ran_remain{Nr+1});
