...
parfor Nt = min_num_tars : max_num_tars
    for i = 1 : N_trials
        ...
        % risk-aware 
        [riskaware(i,Nt), riskaware_remain_bestremo(i,Nt), bestremo_rate_riskaware(i,Nt)] = riskaware_opt_remo_fun(Nr, N_direction,...
...
title('numbers of tracked targets after optimal removal','fontsize',12)
legend('brute-force','resilient','risk-aware','greedy','random');
xlabel('number of targets','fontsize',11)
ylabel('coverage number','fontsize',11)
...
shadedErrorBar(min_num_tars:max_num_tars,bestremo_rate_riskaware(:,min_num_tars:max_num_tars),...
    {@mean,@std},'lineprops','.-c','patchSaturation',0.33);
