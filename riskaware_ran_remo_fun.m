function [N_greedy_cover, gre_remain_ran_remo, ran_remo_rate_gre] = riskaware_ran_remo_fun(Nr, N_direction,...
    N_failure, tar_cover, r_set, r_ran_remo, prob_attack)

    % calculate the risk-aware greedy set
    ...
    for k = 1:Nr % we need to greedily pick Nr rounds
        ...
        for i = 1:length(remain_set)
            for j = 1:N_direction
                marg_gain = (1 - prob_attack(remain_set(i))) * (length(union(tar_cover{remain_set(i),j},s_greedy{k})) - length(s_greedy{k})); 
...
%                            s_greedy_index,N_greedy_cover);
    ...
    % random removal            
    [N_greedy_cover, gre_remain_ran_remo, ran_remo_rate_gre] = random_removal(Nr, tar_cover, ...
        s_greedy_index, r_ran_remo);
end