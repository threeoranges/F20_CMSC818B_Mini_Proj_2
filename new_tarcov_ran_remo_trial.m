...
parfor Nt = min_num_tars : max_num_tars
    for i = 1 : N_trials
        ...
        % risk-aware 
        [riskaware(i,Nt), riskaware_remain_ranremo(i,Nt), ranremo_rate_riskaware(i,Nt)] = riskaware_ran_remo_fun(Nr, N_direction,...
            N_failure, tar_cover, r_set, r_ran_remo, prob_attack);
...
shadedErrorBar(min_num_tars:max_num_tars,riskaware_remain_ranremo(:,min_num_tars:max_num_tars),...
...
title('numbers of tracked targets after random removal','fontsize',12)
legend('brute-force','resilient','risk-neutral','greedy','random')
xlabel('number of targets','fontsize',11)
ylabel('coverage number','fontsize',11)
...
shadedErrorBar(min_num_tars:max_num_tars,ranremo_rate_riskaware(:,min_num_tars:max_num_tars),...
    {@mean,@std},'lineprops','.-c','patchSaturation',0.33);
